...
def create_constraints(tx):
    ...
    # Create fulltext index for job text search
    try:
        tx.run("""
            CREATE FULLTEXT INDEX jobTextIndex IF NOT EXISTS
            FOR (j:JOB) ON EACH [j.text]
        """)
        print("Fulltext index 'jobTextIndex' created.")
    except Exception as e:
        print(f"Fulltext index creation failed: {e}")
    
    # Create vector index for job semantic search (384 dimensions for all-MiniLM-L6-v2)
    try:
        tx.run("""
        ...
        """)
        print("Vector index 'jobEmbeddingIndex' created.")
    except Exception as e:
        print(f"Job vector index creation failed: {e}")
